...
RUN apt-get install -y wget curl tar clang rpm gfortran build-essential libpcre3-dev autoconf automake libtool nasm gcc-multilib g++-multilib python3.9 python3.9-dev
RUN apt-get install -y docbook-xml docbook-xsl xsltproc libxml2-dev libxslt-dev fop docbook-xsl-doc-pdf bison subversion git default-jdk python3-distutils swig python3-gdbm gettext zip flex
...
RUN ln -s /usr/bin/python3.9 /usr/bin/default_python
COPY sitecustomize.py /usr/lib/python3.9/sitecustomize.py
